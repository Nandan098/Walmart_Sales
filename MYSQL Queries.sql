...
-- Q-2. Identify the highest-rated category in each branch, Display the branch, category, and avg rating
		...
		from  (select branch, category, round(avg(rating),2) as avg_rating,rank() over(partition by branch order by avg(rating) desc) as Ranks
               from wallmart 
...
-- Q3: Identify the busiest day for each branch based on the number of transactions
       select t.branch,t.day_name,t.no_of_transaction
	   from (select branch,dayname(date) as day_name,count(*) as no_of_transaction,rank() over(partition by branch order by count(*) desc) as ranks
             from wallmart
...
-- Q9: Identify the 5 branches with the highest revenue decrease ratio from last year to current year (e.g., 2022 to 2023)
       ...
       select r1.branch, r1.revenue_2022 as prev_revenue,
	   r2.revenue_2023 as current_revenue,
	   round((r1.revenue_2022-r2.revenue_2023)*100.0/r1.revenue_2022,2) as revenue_decrease_ratio
